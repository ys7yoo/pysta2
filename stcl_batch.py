...
# # STC + Clustering
# ###############################################################################
# def run_stcl(stim, spike_counts, info, tap=8, cov_algorithm="classic", cluster_dim=1, save_folder_name="clustering"):
#
#     channel_names = list()
#     num_spikes = list()
#     sta_PSNR = list()
...
if __name__ == '__main__':
    ...
    data_path = "data/binary_stim"
    stim, spike_counts, info = pysta.load_data(dataset, data_path)

    results_path = "results/binary_stim_{}_tap{}_cov_{}_cluster_dim{}".format(dataset, args.tap, args.cov_algorithm, args.dim)
    if not os.path.exists(results_path):
        os.makedirs(results_path)

    stcl.run(stim, spike_counts, info['channel_names'],  # info["cell type"],
             tap=args.tap, cov_algorithm=args.cov_algorithm, cluster_dim=args.dim, results_path=results_path)
