...
class BiGauss(object):
    """
    Implementation of Measuring the component overlapping in the Gaussian mixture model by Sun and Wang (2011)
    Modified code from https://github.com/SJinping/GMM-overlap-rate
    """
...
def calc_overlap_rate(mu1, mu2, Sigma1, Sigma2, pi1, pi2):
    bg = BiGauss(mu1, mu2, Sigma1, Sigma2, pi1, pi2)
    return bg.overlap_rate()


# def calc_overlap_rate(pi1 = 0.5, mu1 = [0, 0], Sigma1 = [[1, 0], [0, 1]], pi2=0.5, mu2 = [3, 0], Sigma2 = [[2.17, 1.82], [1.82, 2.17]]):
#
#     Bi = BiGauss(mu1, mu2, Sigma1, Sigma2, pi1, pi2)
#     steps = Bi.steps
#
#     e = math.sqrt((mu1[0] - mu2[0]) ** 2 + (mu1[1] - mu2[1]) ** 2) / float(steps)
#     x_step = e * (mu1[0] - mu2[0])  # each step for x
#     y_step = e * (mu1[1] - mu2[1])  # each step for y
#     p_x = mu1[0] - x_step
#     while Bi.RC(p_x) == None:
#         p_x = p_x - x_step
#     p_y = Bi.RC(p_x)
#     p = [p_x, p_y]
#     p_pre = mu1
#     p_min = min(Bi.pdf(p), Bi.pdf(p_pre))
#     p_max = max(Bi.pdf(p), Bi.pdf(p_pre))
#     index = 0
#     while index < steps:
#         if Bi.RC(p[0] - x_step) != None:
#             p_next = [p[0] - x_step, Bi.RC(p[0] - x_step)]  # next point on ridge curve
#             if Bi.pdf(p) > Bi.pdf(p_pre) and Bi.pdf(p) > Bi.pdf(p_next):
#                 p_max = Bi.pdf(p)
#             if Bi.pdf(p) < Bi.pdf(p_pre) and Bi.pdf(p) < Bi.pdf(p_next):
#                 p_min = Bi.pdf(p)
#         p_pre = p
...
#     pdf_mu1 = Bi.pdf(mu1)
#     pdf_mu2 = Bi.pdf(mu2)
#     print('the OLR of this mixture Gaussian: {}'.format(
#         p_min / min(pdf_mu1, pdf_mu2) if p_min < min(pdf_mu1, pdf_mu2) else 1.0))
...
#     Bi = BiGauss(mu1, mu2, Sigma1, Sigma2, pi1, pi2)
#     print('the OLR of this mixture Gaussian: {}'.format(Bi.overlap_rate()))
#
#     exit(0)
#
#     # OLR's changing over distance between two means (Fig. 7)
#     step = 0.1
#     pi1 = 0.5
#     OLRs = []
#     x = []
